...
# Install ClassiCube on OS X Tiger / PowerPC.
...
# Use our libcurl.
CFLAGS="$CFLAGS -I/opt/curl-7.87.0$ppc64/include"
LDFLAGS="$LDFLAGS -L/opt/curl-7.87.0$ppc64/lib -lcurl"

# Target OpenGL 1.1
make $(tiger.sh -j) CC="$CC" CFLAGS="$CFLAGS -DCC_BUILD_GL11" LDFLAGS="$LDFLAGS"
mv ClassiCube ClassiCube-OpenGL1.1

# Target OpenGL 1.5
make clean || true
make $(tiger.sh -j) CC="$CC" CFLAGS="$CFLAGS" LDFLAGS="$LDFLAGS"
