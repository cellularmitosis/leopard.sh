...
def fix_linker_check(lines):
    lines2 = []
    for line in lines:
        if "tiger.sh --arch-check $pkgspec $ppc64" in line:
            lines2.append("    tiger.sh --linker-check $pkgspec")
        lines2.append(line)
    return lines2
...
def fix_shebang(lines):
    lines2 = []
    for line in lines:
        if line == "#!/bin/bash":
            line = "#!/opt/tigersh-deps-0.1/bin/bash"
        lines2.append(line)
    return lines2
...
def fix_path(lines):
    lines2 = []
    for line in lines:
        if "PATH=" in line and "MPN_PATH" not in line and "tigersh-deps-0.1" not in line:
...
def fix_mirror(lines):
    lines2 = []
    for line in lines:
        if "TIGERSH_MIRROR=" in line:
            line = "TIGERSH_MIRROR=${TIGERSH_MIRROR:-https://leopard.sh}"
        lines2.append(line)
    return lines2
...
"tiger.sh --unpack-dist $pkgspec" ]:
                lines2.append(l)
            while "cd $package-$" not in lines[i] and "cd ncurses-$version" not in lines[i]:
                i += 1
            continue
...
def fix_config_cache2(lines):
    lines2 = []
    for line in lines:
        if "cat /opt/tiger.sh/share/tiger.sh/config.cache" in line:
            continue
        else:
            lines2.append(line)
    return lines2
