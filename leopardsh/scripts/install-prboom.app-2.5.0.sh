...
# Install PrDoom.app on OS X / PowerPC.

# Note: the PrBoom "launcher" app doesn't seem to work (the "launch" button
# stays disabled no matter what you do).  However, you can still open a terminal
# and run /opt/prboom.app-2.5.0/PrBoom.app/Contents/MacOS/PrBoom

package=prboom.app
version=2.5.0
appname1=PrBoom
upstream=https://versaweb.dl.sourceforge.net/project/prboom/prboom%20stable/2.5.0/PrBoom-2.5.0.dmg
...
if ! test -e "$HOME/Library/Application Support/PrBoom/doom1.wad" ; then
    mkdir -p "$HOME/Library/Application Support/PrBoom"
    ln -s /opt/doom-shareware-wad-1.9/share/doom/doom1.wad "$HOME/Library/Application Support/PrBoom/"
fi
